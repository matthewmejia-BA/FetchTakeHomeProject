...
Secondly, I decided to implement a random recipe feature to help the user decide more quickly if they don't have a preference. One issue I came across but don't have time to troubleshoot, is when the app is on the random recipe view, app goes into background mode, and once the app comes to foreground the random recipe is not the same. My solution would be to store the random recipe and only trigger to update it via button. 
...
I wanted to use modern SwiftUI and with the latest Swift 6 version that helps with concurrency. Unfortunatly the tradeoff with this approach is that earlier iOS versions ( < 17.0 ) aren't compatible. 
